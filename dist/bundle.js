...
function cards() {
  ...
  Object(_services_services__WEBPACK_IMPORTED_MODULE_0__["getResource"])('http://localhost:3000/menu') //настраиваем связь - клиент/сервер
  .then(data => {
...
function forms(formsSelector, modalTimerId) {
  ...
  function bindPostData(form) {
    form.addEventListener('submit', e => {
      //форма которая будет отправляться по подтверждению 
      e.preventDefault();
      ...
      statusMessage.style.cssText = `
            ...
            margin: 0 auto;
        `;
      form.insertAdjacentElement('afterend', statusMessage); // const request = new XMLHttpRequest();               //привязка к серверу через пхп 
      ...
      const json = JSON.stringify(Object.fromEntries(formData.entries())); //разобрали данные, собрали в массив обратно и перевели в формат джейсон (справа на лево)
      // const object = {};
...
function modal(triggerSelector, modalSelector, modalTimerId) {
  ...
  function showModalByScroll() {
    //вызываем м_окно когда доскролили до низа  страницы 
...
function slider({
  ...
  const slides = document.querySelectorAll(slide),
        ...
        //для точек перебора слайдов 
  prev = document.querySelector(prevArrow),
        next = document.querySelector(nextArrow),
        total = document.querySelector(totalCounter),
        //отображает изменение цифр слайдов 
  ...
  if (slides.length < 10) {
    //меняем цифри слайдов и текущее значение 
    total.textContent = `0${slides.length}`; //current.textContent =  `0${slideIndex}`;
  } else {
  ...
  const indicators = document.createElement('ol'),
        //для точек слайда
  dots = [];
  indicators.classList.add('carousel-indicators'); //прописываем свойства тут из CSS
  ...
  next.addEventListener('click', () => {
    //незабываем что width - это строка и меняем ее на число
    ...
    formatText(); // dots.forEach(dot => dot.style.opacity = ".5");
    // dots[slideIndex-1].style.opacity = 1;
  ...
  dots.forEach(dot => {
    dot.addEventListener('click', e => {
      ...
      slidesField.style.transform = `translateX(-${offset}px)`; // if (slides.length < 10) {
      //     current.textContent =  `0${slideIndex}`;
      // } else {
      //     current.textContent =  slideIndex;
      // }
...
function timer(id, deadline) {
  ...
  function getZero(num) {
    //подставления ноля перед однозначной датой (09 дней)
    if (num >= 0 && num < 10) {
      return `0${num}`;
    } else {
      return num;
...
window.addEventListener('DOMContentLoaded', () => {
  const modalTimerId = setTimeout(() => Object(_modules_modal__WEBPACK_IMPORTED_MODULE_2__["openModal"])('.modal', modalTimerId), 50000); //открывает автоматически мод_окно 
  ...
  Object(_modules_timer__WEBPACK_IMPORTED_MODULE_3__["default"])('.timer', '2021-05-11');
...
const postData = async (url, data) => {
  //меняем асинхронность выполнения async/await
...
const getResource = async url => {
  ...
  if (!res.ok) {
    //прописали руками ошибки на которые не реагирует фетч
